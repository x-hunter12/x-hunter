...
if [[ 0 -eq $? ]]; then
echo -e "${y} ┌──────────────────────────────────────────┐${NC}"
echo -e "${y} │${NC} $1 berhasil dipasang"
echo -e "${y} └──────────────────────────────────────────┘${NC}"
sleep 2
fi
...
wget -q ${REPO}autopointing.sh && chmod +x autopointing.sh && ./autopointing.sh
...
wget -q -O xdxl-htvpn.sh "${REPO}xdxl-htvpn.sh"
chmod +x xdxl-htvpn.sh
bash xdxl-htvpn.sh
rm -fr xdxl-htvpn.sh
...
wget -q -O /tmp/nameserver "${REPO}nameserver.sh" >/dev/null 2>&1
...
wget -q -O openvpn.sh "${REPO}openvpn.sh" &&  chmod +x openvpn.sh && ./openvpn.sh
...
clear
# banner
echo "Banner /etc/banner/banner.com" >>/etc/ssh/sshd_config
sed -i 's@DROPBEAR_BANNER=""@DROPBEAR_BANNER="/etc/banner/banner.com"@g' /etc/default/dropbear

# Ganti Banner
wget -q -O /etc/banner/banner.com "${REPO}issue.net"
...
wget -q -O /usr/bin/ws-epro "${REPO}ws-epro" >/dev/null 2>&1
wget -q -O /usr/bin/tun.conf "${REPO}tun.conf" >/dev/null 2>&1
wget -q -O /etc/systemd/system/ws-epro.service "${REPO}ws-epro.service" >/dev/null 2>&1
...
END
rm -fr /root/xdxl/.isp
rm -fr /root/xdxl/.city
rm -fr /root/xdxl/.timezone
